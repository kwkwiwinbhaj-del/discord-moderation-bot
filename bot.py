...
async def on_member_update(before, after):
    """Handle server tag (identity) changes"""
    guild = after.guild
    ...
    try:
        before_has_tag = before.premium_since is not None if hasattr(before, 'premium_since') else False
        after_has_tag = after.premium_since is not None if hasattr(after, 'premium_since') else False
        
        if not before_has_tag and after_has_tag and role not in after.roles:
        ...
        elif before_has_tag and not after_has_tag and role in after.roles:
